...
export function breadcrumbsForSlug(menu, slug, path) {
    path = path ? path : []
    menu.forEach(obj => {
        if (urlInChildren(obj, slug)) {
            path.push({ name: obj.name, url: obj.url })
...
export function urlInChildren(menu, slug) {
    let found = false
...
export function getPreviousNextFromMenu(menu, slug, prev, next) {
    let current
    
    // See if our slug is on this level
