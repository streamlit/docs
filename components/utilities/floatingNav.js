...
export default class FloatingNav extends React.Component {
    ...
    render() {
        ...
        let svg = (
            <svg xmlns="http://www.w3.org/2000/svg" width="6" height="10" viewBox="0 0 6 10" fill="none"><path d="M0.952437 7.90752L1.14007 8.07272L0.952436 7.90752C0.682521 8.21409 0.682521 8.69954 0.952436 9.00611C1.23875 9.3313 1.71774 9.3313 2.00405 9.00611L5.04756 5.54929C5.31748 5.24273 5.31748 4.75727 5.04756 4.45071L2.00405 0.993892C1.71774 0.668703 1.23875 0.668703 0.952437 0.993892C0.682521 1.30046 0.682521 1.78591 0.952437 2.09248L3.51233 5L0.952437 7.90752Z" fill="none" stroke="none" stroke-width="0.5"></path></svg>
        )
        return (
            <div className='toc'>
                <ol className='toc-level'>
                    {menu.map((item, index) => {
                        ...
                        return (<li className={`level-${item.level} ${active} ${location}`} key={`toc-${index}`}>{svg}<a href={item.target}>{item.label}</a></li>)
                    })}
                </ol>
            </div>
        )
