...
export default class QuickLink extends React.Component {
    ...
    autoScroll() {
        if (window.location.hash == `#${this.state.cleanID}`) {
            const element = document.getElementById(this.state.cleanID);
            window.scrollTo(0, element.offsetTop - 65);
        }
    }

    async copyLink() {
        const link = `${window.location.host}${window.location.pathname}#${this.state.cleanID}`
        await navigator.clipboard.writeText(link);
        window.location.hash = this.state.cleanID;
        setTimeout(this.autoScroll, 100);
    }
