...
- 🔀 Streamlit now supports `async`/`await` in app code, so you can use async libraries like httpx, aiohttp, and asyncpg directly ([#16167](https://github.com/streamlit/streamlit/pull/16167), [#744](https://github.com/streamlit/streamlit/issues/744)).
    - [`st.cache_data`](/develop/api-reference/caching-and-state/st.cache_data) and [`st.cache_resource`](/develop/api-reference/caching-and-state/st.cache_resource) work with `async def` functions and cache the awaited result ([#16168](https://github.com/streamlit/streamlit/pull/16168)).
