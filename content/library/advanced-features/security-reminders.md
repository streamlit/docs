...
Streamlit's [`st.cache_data`](/develop/concepts/execution-model/caching#stcache_data) and [`st.session_state`](/develop/concepts/execution-model/session-state#serializable-session-state) implicitly use the `pickle` module, which is known to be insecure. It is possible to construct malicious pickle data that will execute arbitrary code during unpickling. Never load data that could have come from an untrusted source in an unsafe mode or that could have been tampered with. **Only load data you trust**.
