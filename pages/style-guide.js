        ...
        <Layout>
            <article className="page container">
                ...
                <Component label=".large P">
                    <p className="large">The easiest way to learn how to use Streamlit is to try things out yourself. As you read through this guide, test each method. </p>
                </Component>
                <Component label="P">
                    <p>Streamlit is more than just a way to make data apps, it’s also a community of creators that share their apps and ideas and help each other make their work better.</p>
                </Component>
                <Component label=".small P">
                    <p className="small">For this guide we’re using small amounts of data so that we can move quickly. You can check out our Tutorial on creating a data explorer to see an example of how to load data from an API and use  @st.cache to cache it.</p>
                </Component>
                <Component label="Inside Link">
                    <p>Check out our <Link href="/"><a>community forum</a></Link> and post a question</p>
                </Component>
                <Component label="Ordered List - Plain">
                    <ol>
                        <li>Create a new Python file named <span className="inline_code">first_app.py</span>, then open it with your IDE or text editor.</li>
                        <li>Next, import Streamlit.</li>
                    </ol>
                </Component>
                <Component label="Ordered List - Tutorial">
                    <ol className="tutorial">
                        <li>The first step is to create a new Python script. Let’s call it <span className="inline_code">uber_pickups.py</span>.</li>
                        <li>Open <span className="inline_code">uber_pickups.py</span> in your favorite IDE or text editor, then add these lines:</li>
                    </ol>
                </Component>
                <Component label="Unordered List">
                    <ul>
                        <li>Check out our <Link href="/"><a>community forum</a></Link> and post a question</li>
                        <li>Quick help from command line with  <span className="inline_code">$ streamlit --help</span></li>
                        <li>
                            Read more documentation! Check out:
                            <ul>
                                <li><Link href="/"><a>Streamlit Cookbook</a></Link> for things like caching and inserting elements out of order</li>
                                <li><Link href="/"><a>API reference</a></Link> for examples of every Streamlit command</li>
                            </ul>
                        </li>
                    </ul>
                </Component>
                <Component label="Code snippet">
                    <Code language="python" code={`import streamlit as st
                        # To make things easier later, we're also importing numpy and pandas for
                        # working with sample data.`} />
                    <Code language="bash" code={`myscript.sh
ls -l myscript.sh`} />
                </Component>
                ...
                <Component label="Note Block">
                    <Note background="l-blue-10" color="gray-90" dark={{ background: 'dark-black', color: 'white' }}>
                        <IconHeader icon="push_pin" rotate="45" title="Note" background="l-blue-70" color="white" />
                        <p>For this guide we’re using small amounts of data so that we can move quickly. You can check out our <Link href="/"><a>Tutorial</a></Link> on creating a data explorer to see an example of how to load data from an API and use  <Link href="/"><a>@st.cache</a></Link> to cache it.</p>
                    </Note>
                </Component>
                <Component label="Note Block (with code embed option)">
                    <Note background="violet-10" color="gray-90" dark={{ background: 'dark-violet', color: 'white' }}>
...
https://raw.githubusercontent.com/streamlit/demo-uber-nyc-pickups/master/streamlit_app.py`} />
                ...
                <Component label="YouTube Embed">
                    <YouTube video_id="BuD3gILJW-Q" caption="Caption can go here if necessary" />
                </Component>
                <Component label="Image">
                    <Image src="/embed-image-01.jpg" alt="Streamlit's email highlighted on their GitHub profile" caption="Caption can go within the frame if necessary" />
                    <Image src="/embed-image-02.png" alt="Streamlit's 'your apps' page highlighting the 'Edit secerets' link" caption="Caption can go within the frame if necessary" />
                </Component>
                <Component label="1, 2 or 3-Up Tiles">
                    <TilesContainer>
                        <Tile size="third" background="l-blue-70" color="white" icon="downloading" title="Install Streamlit" text="Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia." />
                        <Tile />
                        <Tile />
                ...
                <Component label="Code Highlights">
                    <Code language="python" lines="11-17" code={`import streamlit as st
import pandas as pd
import numpy as np
...
data_load_state.text("Done! (using st.cache)")`} />
                ...
                <Component label="Show Component Alongside Code">
                    <Code img="/data-table.png" language="python" code={`chart_data = pd.DataFrame(
    p.random.randn(20, 3),
    columns=['a', 'b', 'c']
)
